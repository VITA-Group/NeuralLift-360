...
def __render_rays_test(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    """
    exp_step_factor = 0 # kwargs.get('exp_step_factor', 0.)
    results = {}
    ...
    while samples < kwargs.get('max_samples', MAX_SAMPLES):
        ...
        normals[valid_mask] = _normals.float()
    ...
    rgb_bg = kwargs.get('bg_color', None)
...
def __render_rays_train(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    sigmas, rgbs, normals = model(xyzs, dirs, **kwargs)
    ...
    results['mask'] = results['opacity'] > 1e-4
